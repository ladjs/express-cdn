...
var renderTag = function(options, assets, attributes) {
  ...
  if (options.production) {
    ...
    if (typeof assets === 'object') {
      ...
      return createTag(src, "/" + name, attributes) + newline;
    } else {
      timestamp = fs.statSync(path.join(options.publicDir, assets)).mtime.getTime();
      position = assets.lastIndexOf('.');
      return createTag(src, _(assets).splice(position, 0, '.' + timestamp), attributes) + newline;
    }
  } else {
    ...
    if (typeof assets === 'object') {
      ...
      return createTag(src, assets, attributes, version) + newline;
    } else {
