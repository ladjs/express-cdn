      ...
      case 'minify':
        ...
        minify.replace(/(background\-image|background|content)\:[^;]*url\((?!data:)['"]?([^\)'"]+)['"]?\)/g, function (match, attribute, url) {
          if (options.production) {
            ...
            var imageResource = compile(relativePath, relativePath, S3, options, 'image', 'image/'+path.extname(url).substr(1), Date.now(), null, null)();
        ...
        // Process fonts
        minify.replace(/(src)\:[^;]*url\((?!data:)['"]?([^\)'"]+)['"]?\)/g, function (match, attribute, url) {
          if (options.production) {
            var relativePath = url;
            if ('/' === relativePath[0]) {
              relativePath = relativePath.substr(1);
            ...
            var fontResource = compile(relativePath, relativePath, S3, options, 'font', mimeType, Date.now(), null, null)();
            return attribute+':url('+url+')';
          } else {
            return attribute+':url('+url+')';
          }
    ...
    if (response.statusCode === 200) {
      ...
      switch(type) {
        ...
        case 'text/javascript':
          ...
          return;
        case 'text/css':
          readUtf8(path.join(options.publicDir, assets), compile(fileName, assets, S3, options, 'minify', type, null, finishUpload));
          // * #change @alexloi : Don't need to minify / Causes error!
          // Changed
          // readCSS(url.format({
...
var processAssets = function(options, results, done) {
  ...
  var S3 = knox.createClient({
    ...
    , endpoint: options.endpoint || (options.bucket + '.s3.amazonaws.com')
  });
      ...
      walker.on('end', function() {
        ...
        for (var i=0; i<results.length; i+=1) {
          ...
          if(typeof results[i] === 'string'){
            ...
            throw new Error('Express CDN: Attributes must be strings');
          }
